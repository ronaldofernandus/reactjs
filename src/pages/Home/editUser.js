...
const EditUser = () => {
  ...
  const { DETAIL_USERResult, updateUserReducerResult } = useSelector(
  ...
  const editHandler = (event) => {
    ...
    Swal.fire({
      ...
      text: `You've successfully Edit User!`,
    });
    navigate("/");
  };

  useEffect(() => {
    if (DETAIL_USERResult) {
      setName(DETAIL_USERResult.name);
      setEmail(DETAIL_USERResult.email);
      setGender(DETAIL_USERResult.gender);
      setStatus(DETAIL_USERResult.status);
      setId(DETAIL_USERResult.id);
      dispatch(getUser());
    }
  }, [DETAIL_USERResult, dispatch]);
  useEffect(() => {
    if (updateUserReducerResult) {
      // console.log("5. Masukk Component did update");
      dispatch(getUser());
    }
  }, [updateUserReducerResult, dispatch]);
